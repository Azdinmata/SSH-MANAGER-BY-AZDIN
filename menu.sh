...
for component in colors banner buttons cards; do
    if [ ! -f "$UI_DIR/$component.sh" ]; then
        curl -fsSL -o "$UI_DIR/$component.sh" "$BASE_UI_URL/$component.sh" 2>/dev/null
    fi
    source "$UI_DIR/$component.sh" 2>/dev/null
done
...
menu_users() {
    while true; do
        ...
        btn "1" "Create Unified Account" "(Custom / Lifetime / Limit / Quota)"
        btn "2" "Edit Existing Account"  "(Password, Expiry, Devices)"
        btn "3" "List All Users"         "(Status, Limits, Details)"
        btn "4" "Get Account Credentials" "(Extract Links & Configs)"
        btn "5" "Delete Account"         "(Purge from all protocols)"
        btn_back "Back to Main Dashboard"
        ...
        case "$opt" in
            1)
                ...
                msg_success "User account generated and synchronized!"
...
menu_protocols() {
    while true; do
        ...
        btn "1" "Core Services Health" "(Real-Time Status)"
        btn "2" "Reboot All Protocols" "(SSH, V2Ray, Nginx, WS)"
        btn "3" "BadVPN 7300 Gateway"  "(Status / Diagnostics)"
        btn "4" "UDP Custom Service"   "(Status / State)"
        btn "5" "SlowDNS (DNSTT)"      "(Status & Public Key)"
        btn_back "Back to Main Dashboard"
...
uninstall_all() {
    ...
    crontab -l 2>/dev/null | grep -v "session-watchdog" | crontab - 2>/dev/null || true
...
menu_settings() {
    while true; do
        ...
        btn "1" "Update Pointing Domain" "(Reconfigure Host)"
        btn "2" "Update NS Domain"       "(SlowDNS Nameserver)"
        btn "3" "Renew SSL Certificate" "(Let's Encrypt / Certbot)"
        btn "4" "Purge & Uninstall All"  "(Delete All Files & Reset)"
        btn_back "Back to Main Dashboard"
...
while true; do
    ...
    btn "1" "User & Account Control"  "(Add, Lifetime, Quota, Limits)"
    btn "2" "Protocol & Server Suite" "(SSH-WS, Trojan, VMess, UDP, DNSTT)"
    btn "3" "System & Security Tools" "(Domain, SSL, Uninstall)"
    btn "4" "Active Sessions Monitor" "(Live Online SSH Connections)"
    btn_back "Exit Interface"
...
print_credentials_card() {
    ...
    echo -e "${C_CYAN}║${C_RESET}     UDP Cust : 1-65535                                                       ${C_CYAN}║${C_RESET}"
    echo -e "${C_CYAN}║${C_RESET}     Payload  : GET / HTTP/1.1[crlf]Host: $SERVER_DOMAIN[crlf]Upgrade: websocket[crlf][crlf] ${C_CYAN}║${C_RESET}"
    echo -e "${C_CYAN}╠══════════════════════════════════════════════════════════════════════════════╣${C_RESET}"
...
uninstall_all() {
    ...
    echo -e "${C_RED}[*] Purging users created by script...${C_RESET}"
    if [ -f "$DB_FILE" ]; then
        while IFS=: read -r u _rest; do
            [[ -n "$u" && ! "$u" =~ ^# ]] && userdel -f "$u" 2>/dev/null || true
        done < "$DB_FILE"
    ...
    echo -e "${C_RED}[*] Purging packages, configs and binaries...${C_RESET}"
    apt-get purge -y nginx v2ray certbot python3-certbot-nginx 2>/dev/null || true
    apt-get autoremove -y --purge 2>/dev/null || true

    rm -rf /etc/ssh-manager
    rm -rf /usr/local/etc/v2ray /etc/v2ray
    rm -rf /root/udp
    rm -rf /etc/nginx/sites-available/* /etc/nginx/sites-enabled/* /etc/nginx/conf.d/*
    rm -rf /etc/letsencrypt

    rm -f /usr/local/bin/ssh-manager
    rm -f /usr/local/bin/menu*
    rm -f /bin/menu /usr/bin/menu
    rm -f /usr/local/bin/badvpn-udpgw
    rm -f /usr/local/bin/udp-custom
    rm -f /usr/local/bin/dnstt-server
    rm -f /usr/local/bin/ws-dropbear
    rm -f /usr/local/bin/session-watchdog
    ...
    echo -e "${C_GREEN}✔ SSH-MANAGER has been completely uninstalled and purged!${C_RESET}"
    exit 0
}
...
menu_settings() {
    while true; do
        ...
        echo -e "  ${C_RED}[4]${C_RESET} ⚠️  Purge & Uninstall Everything (Clean Server)"
        echo -e "  ${C_CYAN}[0]${C_RESET} Back to Main Dashboard"
        echo ""
        read -p "  Action: " opt
        case "$opt" in
...
while true; do
    ...
    echo -e "  ${C_CYAN}[2]${C_RESET} Protocol S    echo -e "              ${C_BOLD}${C_YELLOW}[ SSH-MANAGER By-AZDIN  |  ALL-IN-ONE SUITE v5.0 ]${C_RESET}"
    echo -e "${C_GRAY}────────────────────────────────────────────────────────────────────────────────${C_RESET}"
    local os_info=$(grep -oP "(?<=PRETTY_NAME=\")[^\"]*" /etc/os-release 2>/dev/null || echo "Linux")
    local mem_usage=$(free -m | awk '/Mem:/ {printf "%.1f%%", $3*100/$2}')
    local total_accs=$(wc -l < "$DB_FILE" 2>/dev/null || echo "0")
    ...
    while true; do
        ...
        case "$opt" in
            1)
                ...
                ;;
            2)
                systemctl restart ssh v2ray nginx badvpn udp-custom dnstt 2>/dev/null || true
                echo -e "  ${C_GREEN}All core services restarted.${C_RESET}"
                pause
                ;;
            3)
...
while true; do
    ...
    echo -e "  ${C_CYAN}[3]${C_RESET} System & SSL            ${C_GRAY}(Domain, Certbot, BBR, NS)${C_RESET}"
