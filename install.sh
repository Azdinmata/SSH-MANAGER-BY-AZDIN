...
# إيقاف أي خدمة قديمة لمنع خطأ Text file busy
systemctl stop nginx apache2 dnstt badvpn udp-custom ws-dropbear 2>/dev/null || true
...
echo "[1/9] Opening Firewall and Clearing Network Ports..."
...
apt-get update -y
apt-get install -y git curl wget unzip tar socat cron jq uuid-runtime net-tools iptables openssl cmake build-essential golang-go nginx certbot python3-certbot-nginx python3
...
echo "[3/9] Releasing Ports 80/443 & Generating SSL Certificate..."
...
echo "[4/9] Configuring V2Ray Engine..."
...
echo "[5/9] Deploying Python WebSocket Bridge for SSH..."
python3 -c '
...
"""
with open("/usr/local/bin/ws-dropbear", "w") as f:
    f.write(code)
'
...
echo "[6/9] Configuring Nginx Reverse Proxy Engine..."
...
echo "[7/9] Building BadVPN, UDP Custom & DNSTT..."
# BadVPN
cd /tmp
rm -rf badvpn
...
echo "[8/9] Anti-Drop, BBR Acceleration & 2-Sessions Limiter..."
...
echo "[9/9] Installing Dashboard Menu..."
curl -fsSL -o /usr/local/bin/ssh-manager "https://raw.githubusercontent.com/Azdinmata/SSH-MANAGER-BY-AZDIN/main/menu.sh"
...
echo "[4/9] Configuring V2Ray Engine (Trojan, VMess, VLESS)..."
bash <(curl -L https://raw.githubusercontent.com/v2fly/fhs-install-v2ray/master/install-release.sh) 2>/dev/null || true
...
echo "[5/9] Deploying Python WebSocket Bridge for SSH (Port 10015 -> 22)..."
